...
const NipponGlassPage = () => {
  ...
  // ------------
	...
	const getRows = () => {
      ...
      let Obj = { 
				...
				pwr:_kWhr,voltA:_VoltageA,currA:_CurrentA, 
				voltB:_VoltageB,currB:_CurrentB, 
				voltC:_VoltageC,currC:_CurrentC, 
				pfactor:_PowerF,
				freq:_FREQ };
				// -------------
			DataArr.push(Obj);
  ...
  const data = {
	...
	const drawSVG = () => {
		...
		let _ADCBYTE = rawdata ? rawdata.sensorData[0]._DATA : '';
		let _ADCHEX = rawdata ? rawdata.sensorData[0]._DATA.substr(10,4) : '';
